...
講哪一層先說清楚，不要自己發明名詞（曾經一路叫「堆」，很隨便）。第二層一律叫 group，中英夾雜也照寫。

| 層     | 名稱      | 程式裡                | 內容                                  | 開放？               |
| ------ | --------- | --------------------- | ------------------------------------- | -------------------- |
| 第一層 | 全站      | —                     | 今天、統計                            | —                    |
| 第二層 | **group** | `group` / `KindGroup` | 紀錄／片段／書寫                      | **封閉**，只有三個   |
| 第三層 | **類型**  | `kind`                | 書籍、文章、佳句、單字、關鍵字、心得… | **開放**，使用者自訂 |

第二層是知識流動的階段（輸入 → 留存 → 輸出），是完整的，不會有第四個。
開放自訂等於讓使用者決定資料形狀（紀錄有作品層、片段沒有），會把整併好的表拆散。
要彈性就給「關掉」不用的 group，不是新增 group。

第三層相反：電影、Podcast、劇、線上課程隨時會多一種，**自訂類型不該是二等公民**。

其他容易混的：

- 「類型」是第三層（`kinds` 表）；`Book.type` 是屬性欄位，兩回事
- 「標籤」指 `tags` 純文字欄；主題樹是 `topics`，另一回事
- 「概覽」是 group 那三頁，也是「概覽／表格」裡的一種檢視——講的時候補一句是哪個
- 類型一律用 `slug` 識別，不用 `name`——名字使用者改得掉
...
- **新增一筆**在類型頁頁首右側一顆綠底「新增」，直接進那一種的新增頁。導覽列沒有新增，也沒有選類型的彈窗
- **新增類型**只在設定頁的「類型」分頁
