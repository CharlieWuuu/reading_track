/**
 * Sectigo Public Server Authentication CA EV R36（中介憑證，公開資料，有效期至 2036-03-21）。
 *
 * 讀冊生活的 TLS 沒把這張中介憑證送出來（送的是另一條 Comodo 舊鏈），
 * 瀏覽器與 curl 會自己補上，Node 不會，於是 fetch 直接死在
 * UNABLE_TO_VERIFY_LEAF_SIGNATURE。把缺的這張當成額外信任的憑證帶進去就能驗過，
 * 它自己仍然要能鏈到 Node 內建的根憑證（Sectigo Public Server Authentication Root R46），
 * 所以這不是關掉驗證。
 *
 * 來源：http://crt.sectigo.com/SectigoPublicServerAuthenticationCAEVR36.crt
 */
export const SECTIGO_EV_R36_PEM = `-----BEGIN CERTIFICATE-----
