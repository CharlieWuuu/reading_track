...
describe("addWritingRow", () => {
  it("類型沒有就長一個出來", async () => {
    await addWritingRow(makeWriting({ kind: "週計劃" }));
  ...
  it("「書籍」「文章」不是類型，是在說它有出處——不該長成一個類型", async () => {
    await addWritingRow(makeWriting({ kind: "書籍" }));
  ...
  it("sourceId 指到某一次閱讀時，掛回它屬於的那本書", async () => {
    ...
    await addBookRow(makeBook({ id: reading, title: "來源書" }));

    const writing = makeWriting({ sourceId: reading });
    await addWritingRow(writing);
...
describe("updateWritingRow", () => {
  it("日期清空存回 null", async () => {
    ...
    await updateWritingRow(writing.id, { date: "" });
