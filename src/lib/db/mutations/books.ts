...
async function ensureKeywords(tx: Tx, names: string[]): Promise<void> {
...
async function setBookKeywords(tx: Tx, bookId: string, names: string[]): Promise<void> {
  await ensureKeywords(tx, names);
  await tx.delete(bookKeywords).where(eq(bookKeywords.bookId, bookId));
  if (names.length) {
    await tx.insert(bookKeywords).values(names.map((keyword) => ({ bookId, keyword })));
  }
}
...
export async function addBookRow(book: Book): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    const existing = origin
      ? await tx.select({ bookId: readings.bookId }).from(readings).where(eq(readings.id, origin))
      : [];
      ...
      (
        await tx
          ...
          .values({
            ...
            typeId: await typeIdFor(tx, book.domain, book.subDomain),
            attributeId: await attributeIdFor(tx, book.type),
          })
          .returning({ id: books.id })
      )[0].id;

    await tx.insert(readings).values({ id: book.id, bookId, ...readingValues(book) });
    await setBookKeywords(tx, bookId, names);
  });
}
...
export async function updateBookRow(id: string, patch: Partial<Book>): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    if (patch.domain !== undefined || patch.subDomain !== undefined) {
      bookPatch.typeId = await typeIdFor(tx, patch.domain ?? "", patch.subDomain ?? "");
    }
    if (patch.type !== undefined) bookPatch.attributeId = await attributeIdFor(tx, patch.type);
    ...
    if (patch.keywords !== undefined)
      await setBookKeywords(tx, target.bookId, splitLines(patch.keywords));
  });
}
...
export async function deleteBookRow(id: string): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    await tx.delete(readings).where(eq(readings.id, id));
...
export async function renameBookKeyword(from: string, to: string): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    await ensureKeywords(tx, [to]);
    await tx.delete(bookKeywords).where(eq(bookKeywords.keyword, from));
