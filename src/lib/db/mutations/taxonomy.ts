...
async function upsertType(name: string, parentId: string | null): Promise<string> {
...
export async function typeIdFor(domain: string, subDomain: string): Promise<string | null> {
  const parentName = domain.trim();
  if (!parentName) return null;

  const parentId = await upsertType(parentName, null);
  const childName = subDomain.trim();
  return childName ? upsertType(childName, parentId) : parentId;
}

/** 屬性改成單選了，舊資料若還帶著多行就取第一個 */
export async function attributeIdFor(value: string): Promise<string | null> {
