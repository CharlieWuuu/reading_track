...
import { makeBook, makeWriting, resetIds } from "@/testing/factories";
...
describe("isPrivate", () => {
  it("個別列標了就是私人", () => {
    expect(isPrivate(makeWriting({ private: "是" }))).toBe(true);
  });

  it("認得幾種常見的寫法，大小寫不拘", () => {
    for (const mark of ["是", "y", "YES", "true", "1", "私人"]) {
      expect(isPrivate(makeWriting({ private: mark }))).toBe(true);
    }
  });
...
describe("withPrivacy", () => {
  it("鎖著就把私人的整列拿掉", () => {
    const rows = [makeWriting({ id: "a" }), makeWriting({ id: "b", private: "是" })];
