...
import { bookKeywords, writingKeywords } from "../src/lib/db/schema/keyword-links";
import { books, readings } from "../src/lib/db/schema/reading";
...
import { metrics, writings } from "../src/lib/db/schema/writing";

/**
 * demo 帳號的假資料。書名作者是真的，日期、心得、關鍵字都是編的。
 *
 * 用法：DATABASE_URL='...' npx tsx scripts/seed-demo.ts demo@archivum.test
 * 會先清掉那個帳號名下的所有資料，再重灌——跑幾次結果都一樣。
 */

const email = process.argv[2] ?? "demo@archivum.test";
...
async function main() {
  ...
  if (!user) {
    console.error(`找不到 ${email}，先用 scripts/create-user.ts 建帳號`);
    process.exit(1);
  }
